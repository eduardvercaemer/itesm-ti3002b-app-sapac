...
const ExportXLSX = () => {
  ...
  const fillDays = (worksheet, start, end, rowIndex, days) => {
    ...
    for (let s = 0; s <= delta; s++) {
      ...
      worksheet.getCell(`${current}+${rowIndex}`).value = getDayOfWeekFromDate(
        days[s],
      );
      worksheet.getCell(`${current}+${rowIndex}`).alignment = {
        horizontal: "center",
  ...
  const onClick = async () => {
    ...
    for (let p = 0; p < filteredData.addresses.length; p++) {
      ...
      currWorksheet.getCell("A3").value =
        `INCIDENCIAS DEL ${filteredData.timeFrame}`;
      currWorksheet.getCell("A3").alignment = {
        horizontal: "center",
        vertical: "middle",
      };
      ...
      currWorksheet.getCell("A5").alignment = {
        horizontal: "center",
        vertical: "middle",
      };
      ...
      currWorksheet.getCell("B5").alignment = {
        horizontal: "center",
        vertical: "middle",
      };
      ...
      currWorksheet.mergeCells(
        `${observationDayInChar}5:${observationDayInChar}6`,
      );
      currWorksheet.getCell(`${observationDayInChar}5`).value = "Observaciones";
      currWorksheet.getCell(`${observationDayInChar}5`).alignment = {
        horizontal: "center",
        vertical: "middle",
      };
      ...
      for (let z = 0; z < filteredData.data[p].employees.length; z++) {
        ...
        currWorksheet.getCell(`A${rowIndex}`).alignment = {
          horizontal: "center",
          vertical: "middle",
        };
        ...
        fillIncidences(
          ...
          finalDayInChar,
          rowIndex,
          currentEmployee.incidences,
        );

        currWorksheet.getCell(`${observationDayInChar}${rowIndex}`).value =
          currentEmployee.observations;
        currWorksheet.getCell(`${observationDayInChar}${rowIndex}`).alignment =
    ...
    anchor.setAttribute(
      "download",
      `REG INCIDENCIAS DEL ${filteredData.timeFrame}`,
    );
  ...
  return (
    <button onClick={onClick} className="expButton">
      {" "}
      Exportar{" "}
    </button>
  );
